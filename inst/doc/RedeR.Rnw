...
The R chunks below can be used to illustrate possible usages for RedeR. The full description integrates a case study that soon is going to appear in a new communication!

\begin{itemize}

\item Sample workflow 1: nested networks.

\begin{scriptsize}

1 - start the app:
<<label=start the app, eval=FALSE>>=
rdp <- RedPort() 
calld(rdp)
@
...
<<label=add subgraphs to the app, eval=FALSE>>=
...
<<label=nest subgraphs, eval=FALSE>>=
...
<<label=assign edges to containers, eval=FALSE>>=
mergeOutEdges(rdp)
@

9 - relax the network
<<label=relax the network, eval=FALSE>>=
relax(rdp,50,400)
@

10 - add a color legend (other types are available):
<<label=add a color legend (other types are available), eval=FALSE>>=
...
11 -  select a gene:
<<label=select a gene, eval=FALSE>>=
selectNodes(rdp,"RET")
@

12 - reset graph:
<<label=reset graph, eval=FALSE>>=
resetd(rdp)
@
...
\item Sample workflow 2: clustering.
...
<<label= add graph to the app, eval=FALSE>>=
addGraph(rdp,sg)
@
...
<<label=map the hclust object onto the network, eval=FALSE>>=
...
9 - add color legend:
<<label=add color legend, eval=FALSE>>=
