...
\emph{RedeR} uses different strategies to link R to Java: 
...
The design of the software is depicted from Figure \ref{fig1}. Complex graphs with many attributes can be transferred from-and-to \emph{R} using \emph{addGraph} and \emph{getGraph} functions. \\
...
Within an active interface, then the method 'addGraph' can easily send R graphs to the application. For example, the following chunk adds an \emph{igraph}\cite{igraph} object (Fig.\ref{fig2}):
\begin{small}
...
\end{center}
\caption{\label{fig2}%
A toy example added to \emph{RedeR} by the \emph{addGraph} function.
}
\end{figure}
...
Another strategy is to wrap graphs into containers and then send it to the Java app. Next, the subgraphs g3 and g4 are assigned to different nested structures (Fig.\ref{fig3}).
\begin{small}
...
\end{center}
\caption{\label{fig3}%
Nested graphs in \emph{RedeR} using the command \emph{addGraph}.
}
\end{figure}
...
In Figure \ref{fig5a} the same graph is used to exemplify the community structure mapped by the edge-betweenness function available in RedeR. In Figure \ref{fig5b} these communities are nested to containers, which are objects of the same class of the nodes but with additional behaviors (Fig.\ref{fig5c}). You can build these containers either using \emph{R} or \emph{Java} functions (see options available in the \emph{clustering} main menu and in the shortcuts of the nested objects, i.e., right-click a container).
...
For the next example you will need to reproduce the graph from Figure \ref{fig5b} (or any graph with containers) in the \emph{RedeR} app, then select one of the communities, and run the chunk below:  a simple degree distribution should be plotted in the R side.
\begin{small}
...
The RedeR jar file is already included in the R package and, as usual, to run Java applications your system should have a copy of the JRE (Java Runtime Environment, version>=5).
